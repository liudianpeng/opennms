...
public class WebAssetsRestService extends OnmsRestService {
    ...
    public List<AssetResource> getResources(@PathParam("assetName") final String assetName) {
        return new ArrayList<>(m_assetLocator.getResources(assetName));
    }
    
    @GET
    @Path("{assetName}/{assetName}.{type}")
    public Response getResource(@PathParam("assetName") final String assetName, final String type) {
        final AssetResource resource = m_assetLocator.getResource(assetName, type);
        if (resource == null) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try (final InputStream is = resource.open()) {
            ...
            throw new WebApplicationException(Response.status(Status.INTERNAL_SERVER_ERROR).type(MediaType.TEXT_PLAIN).entity("Resource " + assetName + "/" + type + " exists, but could not be read.").build());
        }
    }
}
