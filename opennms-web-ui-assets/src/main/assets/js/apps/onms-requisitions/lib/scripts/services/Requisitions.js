  ...
  .factory('RequisitionsService', ['$q', '$cacheFactory', '$window', '$http', '$timeout', '$log', function($q, $cacheFactory, $window, $http, $timeout, $log) {
    ...
    */
    requisitionsService.startTiming = function(ts) {
      const t = ts || requisitionsService.internal.defaultTimeout;
      $log.debug('startTiming: starting timeout of ' + t + ' seconds.');
    ...
    */
    requisitionsService.getAvailableLocations = function() {
      ...
      .success(function(data) {
        ...
        var locations = [];
        angular.forEach(data.location, function(loc) {
          if (loc['location-name'] !== 'Default') {
            locations.push(loc['location-name']);
          }
        });
